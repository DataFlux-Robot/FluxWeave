...
from PySide6.QtWidgets import (
    ...
    QComboBox, QHeaderView, QMessageBox, QTabWidget, QGroupBox, QSlider
)
from PySide6.QtCore import QTimer, Qt, Signal
from PySide6.QtGui import QColor, QPalette

from stl_metadata import extract_metadata_text, prepare_stl_for_read, write_embedded_metadata
...
class PartsEditor(QMainWindow):
    ...
    metadataEmbedded = Signal(str)
    xmlExported = Signal(str)
    ...
    def __init__(self):
        ...
        self.active_rotation_tests = {}  # point_index -> {'angle':..,'timer':QTimer,'base':vtkTransform}
        ...
        self.last_metadata_path: Optional[str] = None
    ...
    def _build_ui(self):
        ...
        opacity_layout.addWidget(QLabel("模型透明度:"))
        ...
        left_panel.addLayout(opacity_layout)
        ...
        unit_layout.addWidget(QLabel("连接点坐标单位:"))
        self.coord_unit_combo = QComboBox()
        self.coord_unit_combo.addItems(["米 (m)", "毫米 (mm)"])
        self.coord_unit_combo.currentIndexChanged.connect(self.on_coord_unit_changed)
        unit_layout.addWidget(self.coord_unit_combo)
        ...
        self.chk_embed_metadata = QCheckBox("保存时嵌入到STL")
        self.chk_embed_metadata.setChecked(True)
        self.btn_export_xml = QPushButton("保存配置")
        self.btn_export_xml.clicked.connect(self.export_xml)
        self.btn_stop_tests = QPushButton("停止所有测试")
        self.btn_stop_tests.clicked.connect(self.stop_all_rotation_tests)
        action_layout.addWidget(self.chk_embed_metadata)
        action_layout.addWidget(self.btn_export_xml)
        action_layout.addWidget(self.btn_stop_tests)
    ...
    def _update_opacity_controls(self) -> None:
        if not hasattr(self, "opacity_slider"):
            return
        value = int(round(self.current_opacity * 100))
        value = max(self.opacity_slider.minimum(), min(self.opacity_slider.maximum(), value))
        self.opacity_slider.blockSignals(True)
        self.opacity_slider.setValue(value)
        self.opacity_slider.blockSignals(False)
        if hasattr(self, "lbl_opacity_value"):
            self.lbl_opacity_value.setText(f"{value}%")

    def _apply_opacity_to_actor(self) -> None:
        if not self.stl_actor:
            return
        prop = self.stl_actor.GetProperty()
        prop.SetOpacity(self.current_opacity)
        self.render_window.Render()

    def on_opacity_slider_changed(self, value: int) -> None:
        self.current_opacity = max(0.01, value / 100.0)
        if hasattr(self, "lbl_opacity_value"):
            self.lbl_opacity_value.setText(f"{value}%")
        self._apply_opacity_to_actor()
    ...
    def _coord_unit_scale(self):
        return 1.0 if self.coord_unit == 'm' else 0.001

    def _format_coord(self, meters: float) -> str:
        scale = self._coord_unit_scale()
        display_value = meters / scale if scale else meters
        text = f"{display_value:.6f}"
        if "." in text:
    ...
    def _parse_coord_text(self, text: str) -> float:
        return float(text) * self._coord_unit_scale()

    def on_coord_unit_changed(self, index: int):
        self.coord_unit = 'm' if index == 0 else 'mm'
        for row in range(MAX_POINTS):
            for col in range(3):
                widget = self.points_table.cellWidget(row, 2 + col)
                if not widget:
                    continue
                widget.blockSignals(True)
                widget.setText(self._format_coord(self.point_coords[row][col]))
    ...
    def edit_point_coord(self, row, coord_index, widget):
        try:
            meters = self._parse_coord_text(widget.text())
            self.point_coords[row][coord_index]=meters
            widget.blockSignals(True)
            widget.setText(self._format_coord(meters))
            widget.blockSignals(False)
            if self.point_actors[row]:
    ...
    def apply_point_row(self, row):
        ...
        for c in range(3):
            ...
            try:
                meters = self._parse_coord_text(w.text())
            except ValueError:
                continue
            self.point_coords[row][c]=meters
            w.blockSignals(True)
            w.setText(self._format_coord(meters))
            w.blockSignals(False)
            updated = True
        self._ensure_point_actor(row)
        if updated and self.point_actors[row]:
            self.point_actors[row].SetPosition(*self.point_coords[row])
            self.render_window.Render()
    ...
    def reset_point_row(self, row):
        ...
        for c in range(3):
            widget = self.points_table.cellWidget(row,2+c)
            widget.blockSignals(True)
            widget.setText(self._format_coord(0.0))
            widget.blockSignals(False)
        if self.point_actors[row]:
    ...
    def toggle_point_rotation_test(self, row, on):
        if on:
            self.stop_all_rotation_tests(except_row=row)
            if not self.stl_actor:
                button: QPushButton | None = self.points_table.cellWidget(row, 8)
                if button:
            ...
            if self.stl_actor and self.stl_actor.GetUserTransform():
                base_transform.DeepCopy(self.stl_actor.GetUserTransform())
            else:
                base_transform.Identity()
            self.active_rotation_tests[row] = dict(
                angle=0,
                timer=timer,
                base=base_transform
            )
            ...
            self._stop_single_rotation_test(row)

    def _rotation_axis_vector(self, axis_idx):
        vec = AXIS_VEC.get(axis_idx, (0, 0, 0))
        mag = math.sqrt(vec[0] * vec[0] + vec[1] * vec[1] + vec[2] * vec[2])
        if mag < 1e-9:
            return (0.0, 0.0, 0.0)
        return (vec[0] / mag, vec[1] / mag, vec[2] / mag)

    def _stop_single_rotation_test(self, row, reset_button=True):
        data = self.active_rotation_tests.pop(row, None)
        if not data:
            return
        timer = data.get('timer')
        if timer:
            timer.stop()
        base_transform: Optional[vtk.vtkTransform] = data.get('base')
        if self.stl_actor:
            if base_transform is not None:
                self.stl_actor.SetUserTransform(base_transform)
            else:
    ...
    def stop_all_rotation_tests(self, except_row=None):
        for row in list(self.active_rotation_tests.keys()):
    ...
    def _tick_point_rotation(self, row):
        ...
        ax = self._rotation_axis_vector(axis_idx)
        px, py, pz = self.point_coords[row]

        rotation = vtk.vtkTransform()
        rotation.PostMultiply()
        # Translate to pivot first because VTK applies row-vector style multiplication.
        rotation.Translate(-px, -py, -pz)
        if ax != (0, 0, 0):
            rotation.RotateWXYZ(data['angle'], *ax)
        rotation.Translate(px, py, pz)
    ...
    def nudge_selected_points(self, vec):
        ...
        for i,en in enumerate(self.point_enabled):
            if en:
                ...
                for c in range(3):
                    widget = self.points_table.cellWidget(i,2+c)
                    widget.blockSignals(True)
                    widget.setText(self._format_coord(self.point_coords[i][c]))
                    widget.blockSignals(False)
    ...
    def load_stl(self):
        ...
        if not self._load_metadata_from_stl(path):
            self.lbl_status.setText("当前STL没有连接点信息")

    def load_stl_path(self, path: str, prefer_xml: bool = True, show_message: bool = True) -> bool:
        if not path:
            return False
        if not os.path.exists(path):
            if show_message:
                QMessageBox.warning(self, "提示", f"未找到 STL 文件:\n{path}")
            return False
        self._load_stl_only(path)
        embedded = self._load_metadata_from_stl(path)
        xml_loaded = False
        if not embedded and prefer_xml:
            xml_path = os.path.splitext(path)[0] + '.xml'
            if os.path.exists(xml_path):
                self._load_xml_only(xml_path)
                xml_loaded = True
        if not embedded and not xml_loaded:
            self.lbl_status.setText("当前STL没有连接点信息")
        return True
    ...
    def load_stl_xml(self):
        ...
        if self._load_metadata_from_stl(stl_path):
            return
        self.lbl_status.setText("当前STL没有连接点信息")
        ...
        if os.path.exists(xml_path):
            ...
            QMessageBox.information(self,"提示",f"未找到嵌入信息或对应 XML: {xml_path}")

    def _load_stl_only(self,path):
        # 清除旧 STL
        self.stop_all_rotation_tests()
        if self.stl_actor:
            self.renderer.RemoveActor(self.stl_actor)
        clean_path, cleanup = prepare_stl_for_read(path)
        try:
    ...
    def _reset_points_table(self):
        for row in range(MAX_POINTS):
            checkbox = self.points_table.cellWidget(row, 0)
            if checkbox and checkbox.isChecked():
                checkbox.setChecked(False)
            name_widget = self.points_table.cellWidget(row, 1)
            if name_widget:
                name_widget.blockSignals(True)
                name_widget.setText(f"point{row+1}")
                name_widget.blockSignals(False)
            for col in range(3):
                coord_widget = self.points_table.cellWidget(row, 2 + col)
                if coord_widget:
                    coord_widget.blockSignals(True)
                    coord_widget.setText(self._format_coord(0.0))
                    coord_widget.blockSignals(False)
            axis_widget = self.points_table.cellWidget(row, 5)
            if axis_widget:
                axis_widget.blockSignals(True)
                axis_widget.setCurrentIndex(0)
                axis_widget.blockSignals(False)
            test_button = self.points_table.cellWidget(row, 8)
            if isinstance(test_button, QPushButton) and test_button.isCheckable() and test_button.isChecked():
                test_button.blockSignals(True)
                test_button.setChecked(False)
                test_button.blockSignals(False)
            self.point_names[row] = f"point{row+1}"
            self.point_coords[row] = [0.0, 0.0, 0.0]
            self.point_axis[row] = 0
            self.point_enabled[row] = False
    ...
    def _load_xml_only(self, path):
        try:
            tree = ET.parse(path)
            root = tree.getroot()
            self._apply_xml_root(root, source=os.path.basename(path))
        except Exception as e:
            traceback.print_exc()
            QMessageBox.critical(self,"错误",f"解析 XML 失败: {e}")

    def _load_metadata_from_stl(self, stl_path: str) -> bool:
        ...
        try:
            ...
            QMessageBox.warning(self, "提示", f"嵌入的配置无法解析: {e}")
            return False
        self._apply_xml_root(root, source=f"嵌入数据({os.path.basename(stl_path)})")
        return True

    def _apply_xml_root(self, root: ET.Element, source: str):
        if root.tag != 'urdf_part':
            raise ValueError("不支持的配置格式: 根元素应为 <urdf_part>")
        ...
        if color_elem is not None:
            ...
            try:
                ...
                self.color = [r, g, b]
                self.le_cr.setText(f"{r:.3f}")
                self.le_cg.setText(f"{g:.3f}")
                self.le_cb.setText(f"{b:.3f}")
                self.apply_color()
            except ValueError:
                pass
        ...
        if link is not None:
            ...
            if origin_elem is not None and origin_elem.get('xyz'):
                try:
                    ...
                    self.le_ox.setText(f"{ox:.6f}")
                    self.le_oy.setText(f"{oy:.6f}")
                    self.le_oz.setText(f"{oz:.6f}")
                except ValueError:
                    pass
            if origin_elem is not None and origin_elem.get('rpy'):
                try:
                    ...
                    self.rpy = [rr, rp, ry]
                    self.le_rr.setText(f"{rr:.6f}")
                    self.le_rp.setText(f"{rp:.6f}")
                    self.le_ry.setText(f"{ry:.6f}")
                except ValueError:
                    pass
                ...
                if mass_elem is not None and inertia_elem is not None:
                    try:
                        ...
                        for key in ['ixx','ixy','ixz','iyy','iyz','izz']:
                            val = float(inertia_elem.get(key, '0'))
                            getattr(self, f"le_{key}").setText(f"{val:.6f}")
                            self.manual_inertia[key] = val
                        self.inertia_tabs.setCurrentIndex(1)
                    except ValueError:
                        pass

                density_attr = inertial.get('density')
                if density_attr:
                    try:
                        density_val = float(density_attr)
                        self.le_density.setText(f"{density_val:.6f}")
        ...
        for row in range(MAX_POINTS):
            ...
            self.points_table.cellWidget(row,1).setText(f"point{row+1}")
            for c in range(3):
                widget = self.points_table.cellWidget(row,2+c)
                widget.blockSignals(True)
                widget.setText(self._format_coord(0.0))
                widget.blockSignals(False)
            self.points_table.cellWidget(row,5).setCurrentIndex(0)
            self.point_coords[row] = [0.0, 0.0, 0.0]
            self.point_names[row] = f"point{row+1}"
            self.point_axis[row] = 0

        for idx, pt_elem in enumerate(root.findall('point')):
            if idx >= MAX_POINTS:
                break
            name = pt_elem.get('name', f'point{idx+1}')
            self.points_table.cellWidget(idx, 1).setText(name)
            self.point_names[idx] = name

            xyz_elem = pt_elem.find('point_xyz')
            if xyz_elem is not None and xyz_elem.text:
                try:
                    x, y, z = map(float, xyz_elem.text.strip().split())
                    self.point_coords[idx] = [x, y, z]
                    for c, val in enumerate([x, y, z]):
                        widget = self.points_table.cellWidget(idx, 2 + c)
                        widget.blockSignals(True)
                        widget.setText(self._format_coord(val))
                        widget.blockSignals(False)
                except ValueError:
                    pass
            ...
            if axis_elem is not None:
                try:
                    axis_vals = list(map(float, axis_elem.get('xyz', '1 0 0').split()))
                    if axis_vals == [1.0, 0.0, 0.0]:
                        axis_index = 0
                    elif axis_vals == [0.0, 1.0, 0.0]:
                        axis_index = 1
                    elif axis_vals == [0.0, 0.0, 1.0]:
                        axis_index = 2
                    else:
                        axis_index = 3
                    self.points_table.cellWidget(idx, 5).setCurrentIndex(axis_index)
                    self.point_axis[idx] = axis_index
                except ValueError:
                    pass

            self.points_table.cellWidget(idx, 0).setChecked(True)
            if self.point_actors[idx]:
                self.point_actors[idx].SetPosition(*self.point_coords[idx])

        self.lbl_status.setText(f"已加载配置: {source}")
    ...
    def export_xml(self):
        ...
        if self.chk_embed_metadata.isChecked():
            try:
                xml_text = self._generate_metadata_xml(stl_name)
                write_embedded_metadata(self.stl_file_path, xml_text)
                self.lbl_status.setText(f"已嵌入配置: {os.path.basename(self.stl_file_path)}")
                QMessageBox.information(self, "完成", "配置已嵌入到当前 STL 文件中")
                self.last_metadata_path = self.stl_file_path
                self.metadataEmbedded.emit(self.stl_file_path)
            except ValueError as e:
                QMessageBox.warning(self, "提示", f"无法嵌入配置: {e}\n请考虑改用 XML 保存模式")
        else:
            xml_path, _ = QFileDialog.getSaveFileName(
                self,
                "保存 XML",
                os.path.join(stl_dir, f"{stl_name}.xml"),
                "XML Files (*.xml)"
            )
            if not xml_path:
                return
            self._write_xml(xml_path, stl_name, mirror_mode=False)
            self.last_metadata_path = xml_path
            self.xmlExported.emit(xml_path)
    ...
    def export_mirror(self):
        ...
        if self.chk_embed_metadata.isChecked():
            try:
                xml_text = self._generate_metadata_xml(mname, mirror_mode=True)
                write_embedded_metadata(mirrored_stl, xml_text)
                QMessageBox.information(self, "完成",
                    f"已导出镜像 STL 并嵌入配置:\n{mirrored_stl}")
                self.last_metadata_path = mirrored_stl
                self.metadataEmbedded.emit(mirrored_stl)
            except ValueError as e:
                QMessageBox.warning(self, "提示", f"镜像 STL 嵌入失败: {e}\n将改为输出 XML 配置")
                ...
                self.last_metadata_path = mirrored_xml
                self.xmlExported.emit(mirrored_xml)
        else:
            mirrored_xml = os.path.join(base_dir, f"{mname}.xml")
            self._write_xml(mirrored_xml, mname, mirror_mode=True)
            QMessageBox.information(self,"完成",f"已导出镜像 STL 与 XML:\n{mirrored_stl}\n{mirrored_xml}")
    ...
    def _build_metadata_xml(self, link_name: str, mirror_mode: bool = False) -> ET.Element:
    ...
    def _generate_metadata_xml(self, link_name: str, mirror_mode: bool = False) -> str:
        root = self._build_metadata_xml(link_name, mirror_mode=mirror_mode)
        xml_bytes = ET.tostring(root, encoding='utf-8')
        return '<?xml version="1.0"?>\n' + xml_bytes.decode('utf-8')

    def _write_xml(self, path, link_name, mirror_mode=False):
        try:
            xml_text = self._generate_metadata_xml(link_name, mirror_mode=mirror_mode)
